...
def ignore_first(df: pd.DataFrame) -> pd.DataFrame:
    ...
    """
    # Rank the trajectories for each user
    df['UserTrajectoryRank'] = df.groupby('UserId')['pseudo_session_trajectory_id'].rank(method='first')

    # Rank the entries within each trajectory
    df['EntryRank'] = df.groupby('pseudo_session_trajectory_id')['UTCTimeOffset'].rank(method='first')

    # Count the number of entries in each trajectory
    df['TrajectoryEntryCount'] = df.groupby('pseudo_session_trajectory_id')['UTCTimeOffset'].transform('count')

    # Mark the first trajectory of each user to be ignored if it has only one entry

    # condition = (df['UserTrajectoryRank'] == 1) & (df['TrajectoryEntryCount'] == 1)
    condition = (df['TrajectoryEntryCount'] == 1)
    df.loc[condition, 'SplitTag'] = 'ignore'
